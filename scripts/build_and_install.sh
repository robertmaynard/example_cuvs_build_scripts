...
cmake -S /code/cuvs/c -B "/code/cuvs/c/build/${TARGET_ARCH}" \
      -DCMAKE_CUDA_FLAGS=-w \
      -DCUVSC_STATIC_CUVS_LIBRARY=ON \
      -DCMAKE_PREFIX_PATH="/code/cuvs/cpp/build/${TARGET_ARCH}" \
      -DBUILD_TESTS=OFF

cmake --build "/code/cuvs/c/build/${TARGET_ARCH}" -j16
cmake --install "/code/cuvs/c/build/${TARGET_ARCH}" --prefix /tmp/rapids/
